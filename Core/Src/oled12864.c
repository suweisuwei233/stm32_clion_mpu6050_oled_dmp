...
void OLED_SetPos(unsigned char x, unsigned char y) {
    OLED_WR_Byte(0xb0 + y, OLED_CMD);                       //设置页坐标
    OLED_WR_Byte(((x & 0xf0) >> 4) | 0x10, OLED_CMD);       //设置 列 高四位地址
    OLED_WR_Byte((x & 0x0f), OLED_CMD);                     //设置 列 低四位地址
}
...
void OLED_ShowString(u8 x, u8 y, u8 *chr, u8 Char_Size) {
    unsigned char j = 0;
    //遍历里面的每一个字符然后 一个一个的调用OLED_ShowChar()
    //    \0   -> null
    while (chr[j] != '\0') {
        OLED_ShowChar(x, y, chr[j], Char_Size);
        x += 8;
        //判断是否满了 是否需要换行 y : [0-7] 每2个单位一行 最大是4行
        if (x > 120) {
            x = 0;
            y += 2;
        }
...
/**
 * 实现小恐龙的跳跃 并修改其中小恐龙的中心坐标
 * 小恐龙有32格高
 */
void OLED_DrawJump() {
//    最高可以跳跃32格高

    unsigned int start_point_x = 16;// ->
    unsigned int start_point_y = 6;// ->  6表示 y 是48个像素
//    int speed[] = {1, 2, 4, 4, 5, 5, 4, 4, 2, 1};
    int speed[] = {0, 1, 2, 1, 0};
    uint8_t i, j;
    uint8_t speed_index = 0;
    uint8_t dino_cursor = 0;
    uint8_t dino_byte;


    //添加新的小恐龙的时候 OLED_SetPos(16,y) y的高度限定 y:[6:4]

    for (speed_index = 0; speed_index < 5; speed_index++) {
//        然后添加新的小恐龙
        for (i = 0; i < 2; i++) {
            OLED_SetPos(start_point_x, 6 + i - speed[speed_index]);
            IIC_Start();
            IIC_WriteByte(0x78);
            IIC_WaitAck();
            IIC_WriteByte(0x40);
            ...
            for (j = 0; j < 16; j++) {
                dino_cursor = i * 16 + j;
                dino_byte = DINO[0][dino_cursor];

                IIC_WriteByte(dino_byte);
                IIC_WaitAck();
            }
            IIC_Stop();
        }
...
//OLED_SetPos(16,y) y的高度限定 y:[6:4]
//同时修改结构体
void OLED_Draw_Dino_By_X_Y(unsigned int x, unsigned int y) {
    unsigned int i, j;
    uint8_t dino_cursor, dino_byte;
    Clear_Draw_Container();
    for (i = 0; i < 2; i++) {
        OLED_SetPos(x, y + i);
        IIC_Start();
        IIC_WriteByte(0x78);
